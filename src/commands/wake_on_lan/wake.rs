...
use poise::serenity_prelude::{CreateEmbed, User, Timestamp, Colour, CreateEmbedFooter};
...
struct TimestampBuilderImpl;
impl TimestampBuilder for TimestampBuilderImpl {
...
pub async fn wake(
    ...
    let (embed, magic_packet) = process_wake(ctx.data(), ctx.author(), name, TimestampBuilderImpl {}).await?;
...
async fn process_wake(
    ...
    author: &User,
    machine_name: String,
    timestamp_builder: impl TimestampBuilder
    ...
    let result = if authorized {
        (
            CreateEmbed::default()
                ...
                .footer(CreateEmbedFooter::new("Packet sent:"))
                .timestamp(timestamp_builder.now()),
...
mod tests {
    use super::*;
    use serde_json::json;
    use poise::serenity_prelude::{Colour, CreateEmbedFooter, Member, RoleId, Timestamp, UserId};
    use crate::data::tests::mock_data;
    use crate::services::wake_on_lan::MacAddress;

    struct FakeTimestampBuilder(i64);
    impl TimestampBuilder for FakeTimestampBuilder {
        fn now(&self) -> Timestamp { Timestamp::from_unix_timestamp(self.0).unwrap() }
    }
    ...
    async fn given_dm_call_and_existing_machine_and_user_in_allowed_list_then_should_wake_machine() {
        ...
        let expected_embed = CreateEmbed::default()
            ...
            .footer(CreateEmbedFooter::new("Packet sent:"))
            .timestamp(Timestamp::from_unix_timestamp(12345678).unwrap());
