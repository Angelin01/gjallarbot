...
pub trait ServitorHandler {
	...
	async fn status(&self, unit_name: &str) -> Result<(), ServitorError>;
...
impl HttpServitorController {
	...
	fn check_response(response: Response) -> Result<(), ServitorError> {
...
pub mod tests {
	...
	impl ServitorHandler for MockServitorController {
		...
		async fn status(&self, _unit_name: &str) -> Result<(), ServitorError> {
